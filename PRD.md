...
### 5.2 Database & ORM Policy

- Prisma schema is the **single source of truth** for application-level data models
- Supabase Postgres is the underlying database
- All tables have Supabase RLS enabled
- Prisma models mirror database tables and relations exactly
- Foreign keys use `on delete cascade`
- Indexes on `board_id`, `cell_id`, `owner_id`

> Prisma schema, migrations, and generated client usage are defined in **TECH_REFERENCE.md**.
...
- **Primary data access:** Prisma Client (server-only)
- **Supabase SDK usage:**
  - Auth (session, user)
...
- Supabase RLS remains the final authorization layer

> This section defines **server-side data access responsibilities only**.  
> Client-side caching, synchronization, and UI state management are handled separately via TanStack Query (see Section 8).

- Cache policy: `revalidateTag('board:{id}')`

> Prisma queries, Supabase integration patterns, and API examples are in **TECH_REFERENCE.md**.
...
- **TanStack Query:** client-side server state management
  ...
  - Optimistic updates (e.g., task check/uncheck)
  - Cache invalidation linked to server mutations
